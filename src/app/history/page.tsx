  ...
  return (
    <div className="min-h-screen pb-28 pt-20 px-4 max-w-lg mx-auto">
      ...
      <div className="mb-6 relative z-10">
        <p className="text-white/40 text-sm">{T.history.subtitle}</p>
        <h2 className="text-2xl font-bold text-white mt-0.5">{T.history.title}</h2>
        {records.length > 0 && <p className="text-white/30 text-sm mt-1">{records.length} {T.history.recordsLogged}</p>}
      </div>

      {records.length === 0 ? (
        <div className="glass p-10 text-center relative z-10">
          <p className="text-5xl mb-4">📭</p>
          <p className="text-white font-semibold">{T.history.empty}</p>
          <p className="text-white/40 text-sm mt-1">{T.history.emptyDesc}</p>
        </div>
      ) : (
