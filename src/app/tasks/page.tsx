...
const QUADRANTS = [
  { urgent: true,  important: true,  label: 'Do First',  sub: 'Urgent & Important',     color: '#f87171', bg: 'rgba(248,113,113,0.08)', border: 'rgba(248,113,113,0.2)', icon: '🔥' },
  { urgent: false, important: true,  label: 'Schedule',  sub: 'Not Urgent & Important',  color: '#60a5fa', bg: 'rgba(96,165,250,0.08)',  border: 'rgba(96,165,250,0.2)',  icon: '📅' },
  { urgent: true,  important: false, label: 'Delegate',  sub: 'Urgent & Not Important',  color: '#fbbf24', bg: 'rgba(251,191,36,0.08)',  border: 'rgba(251,191,36,0.2)',  icon: '⚡' },
  { urgent: false, important: false, label: 'Eliminate', sub: 'Not Urgent & Not Important', color: '#94a3b8', bg: 'rgba(148,163,184,0.06)', border: 'rgba(148,163,184,0.12)', icon: '🗑️' },
]
  ...
  return (
    <div className="min-h-screen pb-28 pt-20 px-4 max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between mb-5 relative z-10">
        <div>
          <p className="text-white/40 text-sm">{new Date(date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          <h2 className="text-2xl font-bold text-white mt-0.5">Tasks</h2>
        </div>
      ...
      {total > 0 && (
        <div className="glass p-4 mb-4 relative z-10">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm text-white/60">{done}/{total} completed</span>
      ...
      {showForm && (
        <form onSubmit={addTask} className="glass p-4 mb-4 space-y-3 relative z-10">
          <input type="text" placeholder="Task title..." value={newTitle} onChange={e => setNewTitle(e.target.value)} required autoFocus
            ...
            {QUADRANTS.map(q => {
              ...
              return (
                <button key={q.label} type="button"
                  ...
                  <div>
                    <p className="text-xs font-semibold" style={{ color: sel ? q.color : 'rgba(255,255,255,0.5)' }}>{q.label}</p>
                    <p className="text-xs text-white/25">{q.sub}</p>
                  </div>
                </button>
              )
            })}
      ...
      {total === 0 ? (
        <div className="glass p-10 text-center relative z-10">
          ...
          <p className="text-white/60 text-sm">No tasks for this day</p>
