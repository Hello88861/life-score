  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-10 px-5 py-4 flex justify-between items-center"
        style={{ background: 'rgba(15,12,41,0.8)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-10 flex px-6 py-3 pb-6 gap-2"
        style={{ background: 'rgba(15,12,41,0.9)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        {NAV.map((item) => {
          const active = pathname === item.href
          return (
            <Link key={item.href} href={item.href}
              className="flex-1 flex flex-col items-center py-2 gap-1 rounded-2xl transition-all duration-200"
